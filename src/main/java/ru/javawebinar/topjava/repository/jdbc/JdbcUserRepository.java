...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        ...
        String selectRoles = "SELECT * FROM user_roles";
        jdbcTemplate.query(selectRoles, rs -> {
            map.computeIfAbsent(rs.getInt("user_id"), userId -> EnumSet.noneOf(Role.class))
                    .add(Role.valueOf(rs.getString("role")));
        });

        String selectUsers = "SELECT * FROM users ORDER BY name, email";
        List<User> users = jdbcTemplate.query(selectUsers, ROW_MAPPER);
